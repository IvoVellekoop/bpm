...
for i=1:size(s,1)
    disp(s{i})
    F = Field(getfield(SingleField,s{i}), pixel_size, 0.9, 'um');
    F.gpu_enabled = false;
    tic()
    ...
    clear Edata;
    clear Eins;   
    Eshaped_in = Field(Eshaped_conj,pixel_size, 0.9, 'um');
    clear Eshaped_conj;
...
%  add a mask to prevent overlaping among different foci.
    ...
    %w=w.*BigMask;
    Ein_spiral=Field(w,pixel_size, 0.9, 'um');
    clear w;
    Eout_ShapedSpiral=Ein_spiral.propagate(NscatBig,layer_thick*size(NscatBig,3));
...
%  Prerequisite file: BigMask.
    ...
    Escan = zeros(1144,1144,200);
    EscanIn = zeros(1144,1144,200);
    ...
    for wavenX = MaxXw:-5:-MaxXw
        for wavenY = -MaxYw:5:MaxYw
            ...
            Ephase_field = Field(Ephase_field.data,pixel_size, 0.9, 'um');
            Eout_scanned = Ephase_field.propagate(Nscat,size(Nscat,3)*layer_thick);
    ...
    Escan = SizedArray(Escan, [pixel_size,pixel_size, layer_thick], 'um');
    EscanIn = SizedArray(EscanIn, [pixel_size,pixel_size, layer_thick], 'um');
end
...
for wavenX = MaxXw:-7:-MaxXw
        for wavenY = -MaxYw:7:MaxYw
            ...
            EtiltIn=Etilt.*Eshaped_input.data;
                %Ephase_field = Ephase_field.data.*BigMask;
            EtiltIn = Field(EtiltIn,pixel_size, 0.9, 'um');
    ...
    Escan = SizedArray(Escan, [pixel_size,pixel_size, layer_thick], 'um');  
    EscanInT = SizedArray(EscanInT, [pixel_size,pixel_size, layer_thick], 'um');
end
...
%     end
    ...
    Ephase_field = Field(Ephase_field,pixel_size, 0.9, 'um');
...
function [SingleMask,Radius] = get_Spiral_Mask(CenterOfMass)
